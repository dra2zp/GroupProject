...
def updateDisplay(state):
    dw.fill(dw.black)
    dw.draw(catimage, (state[0], state[1]))
...
def updateState(state):
    if (state[0] > width or state[0] < 0):
        return((state[0],state[1]+state[3],0,state[3]))
    if (state[1] > height or state[1] < 0):
        return((state[0]+state[2],state[1],state[2],0))
    else:
...
# state -> bool
def endState(state):
    if (state[0] > 600 or state[0] < -100 or state[1] > 600 or state[1] < -100):
        return True
    else:
        return False
...
initState = (randint(150,350), randint(150,350), randint(-5,5), randint(-5,5))
